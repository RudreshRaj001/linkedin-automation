...
exports.getRandomTopic = exports.generateImage = exports.generateText = void 0;
...
const generateText = async (req, res) => {
    ...
    const topic = (0, exports.getRandomTopic)(); // Randomly select a topic from your predefined list
    // Removed the 'if (!professionalRole || !topic)' check as they are now defined internally
    // --- CHANGES END HERE ---
    if (!GEMINI_API_KEY) {
        return res.status(500).json({ error: 'Gemini API Key is not configured on the server.' });
    }
    const prompt = (`As a ${professionalRole}, create a highly engaging, innovative, and thought-provoking LinkedIn post ` +
...
// Function to randomly select a topic (can be used by frontend or internally)
const getRandomTopic = () => {
    return POSSIBLE_TOPICS[Math.floor(Math.random() * POSSIBLE_TOPICS.length)];
};
